...
var menu = [
  ...
  },
  {
    ...
    func: function () {
      ...
      if (Galaxy && Galaxy.currHistoryPanel && Galaxy.router) {
        Galaxy.router.push(
          `/histories/sharing?id=${Galaxy.currHistoryPanel.model.id}`
        );
      }
    },
  ...
  },
  {
    ...
    func: function () {
      ...
      if (Galaxy && Galaxy.currHistoryPanel && Galaxy.router) {
        Galaxy.router.push(
          `/histories/permissions?id=${Galaxy.currHistoryPanel.model.id}`
        );
      }
    },
    singleUserDisable: true,
  ...
  },
  {
    ...
    func: function () {
      ...
      if (
        ...
        $.post(
          `${getAppRoot()}history/adjust_hidden`,
          { user_action: "unhide" },
          () => {
            Galaxy.currHistoryPanel.loadCurrentHistory();
          }
  ...
  },
  {
    ...
    func: function () {
      ...
      if (Galaxy && Galaxy.currHistoryPanel && Galaxy.router) {
        Galaxy.router.push(
          `/histories/citations?id=${Galaxy.currHistoryPanel.model.id}`
        );
      }
    },
  },
  {
    ...
    func: function () {
      ...
      if (Galaxy && Galaxy.currHistoryPanel && Galaxy.router) {
        Galaxy.router.push(
          `/histories/${Galaxy.currHistoryPanel.model.id}/export`
        );
      }
    },
  },
    ...
    webhooks.each((model) => {
      ...
      if (webhook.activate) {
        webhooks_menu.push({
          ...
          func:
            webhook.config.function && new Function(webhook.config.function),
          anon: true,
        });
      }
    });
...
function buildMenu(isAnon, purgeAllowed, urlRoot) {
  ...
  return _.clone(menu).filter((menuOption) => {
    ...
    if (menuOption.confirm) {
      menuOption.func = () => {
        const galaxy_main = window.parent.document.getElementById(
          "galaxy_main"
        );
