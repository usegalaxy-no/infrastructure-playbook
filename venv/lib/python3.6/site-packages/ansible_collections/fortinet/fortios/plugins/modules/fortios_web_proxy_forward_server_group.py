...
def web_proxy_forward_server_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('web_proxy', 'forward_server_group', filtered_data, vdom=vdom)
        current_data = fos.get('web_proxy', 'forward_server_group', vdom=vdom, mkey=mkey)
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    fos.do_member_operation('web_proxy_forward_server_group')
    if data['web_proxy_forward_server_group']:
        resp = web_proxy_forward_server_group(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('web_proxy_forward_server_group'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["web_proxy_forward_server_group"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["web_proxy_forward_server_group"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "web_proxy_forward_server_group")
