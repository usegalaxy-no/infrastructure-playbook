...
def filter_firewall_service_group_data(json):
    option_list = ['color', 'comment', 'fabric_object',
                   'member', 'name', 'proxy']
    dictionary = {}
...
def firewall_service_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_service', 'group', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_service', 'group', vdom=vdom, mkey=mkey)
...
def fortios_firewall_service(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_service_group')
