...
def wireless_controller_wtp(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('wireless_controller', 'wtp', filtered_data, vdom=vdom)
        current_data = fos.get('wireless_controller', 'wtp', vdom=vdom, mkey=mkey)
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    fos.do_member_operation('wireless_controller_wtp')
