...
def filter_user_fsso_data(json):
    option_list = ['group_poll_interval', 'interface', 'interface_select_method',
                   'ldap_poll', 'ldap_poll_filter', 'ldap_poll_interval',
                   'ldap_server', 'logon_timeout', 'name',
...
def fortios_user(data, fos, check_mode):
    ...
    fos.do_member_operation('user_fsso')
    if data['user_fsso']:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["user_fsso"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "user_fsso")
