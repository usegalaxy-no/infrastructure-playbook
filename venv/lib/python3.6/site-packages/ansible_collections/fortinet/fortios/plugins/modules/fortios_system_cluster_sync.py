...
def filter_system_cluster_sync_data(json):
    option_list = ['down_intfs_before_sess_sync', 'hb_interval', 'hb_lost_threshold',
                   'ike_heartbeat_interval', 'ike_monitor', 'ike_monitor_interval',
                   'ike_seqjump_speed', 'ipsec_tunnel_sync', 'peerip',
                   'peervd', 'secondary_add_ipsec_routes', 'session_sync_filter',
...
def system_cluster_sync(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'cluster_sync', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'cluster_sync', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_cluster_sync')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_cluster_sync"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_cluster_sync")
