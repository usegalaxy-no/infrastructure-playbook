...
def filter_user_tacacsplus_data(json):
    option_list = ['authen_type', 'authorization', 'interface',
                   'interface_select_method', 'key', 'name',
                   'port', 'secondary_key', 'secondary_server',
                   'server', 'source_ip', 'tertiary_key',
                   'tertiary_server']
    dictionary = {}
...
def user_tacacsplus(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('user', 'tacacsplus', filtered_data, vdom=vdom)
        current_data = fos.get('user', 'tacacsplus', vdom=vdom, mkey=mkey)
