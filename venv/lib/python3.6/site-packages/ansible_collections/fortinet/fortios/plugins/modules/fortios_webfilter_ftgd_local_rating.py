...
def filter_webfilter_ftgd_local_rating_data(json):
    option_list = ['comment', 'rating', 'status',
                   'url']
    dictionary = {}
...
def webfilter_ftgd_local_rating(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('webfilter', 'ftgd_local_rating', filtered_data, vdom=vdom)
        current_data = fos.get('webfilter', 'ftgd_local_rating', vdom=vdom, mkey=mkey)
...
def fortios_webfilter(data, fos, check_mode):
    ...
    fos.do_member_operation('webfilter_ftgd_local_rating')
    if data['webfilter_ftgd_local_rating']:
        resp = webfilter_ftgd_local_rating(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('webfilter_ftgd_local_rating'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["webfilter_ftgd_local_rating"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["webfilter_ftgd_local_rating"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "webfilter_ftgd_local_rating")
