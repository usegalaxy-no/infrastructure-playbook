...
def firewall_ipmacbinding_table(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_ipmacbinding', 'table', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_ipmacbinding', 'table', vdom=vdom, mkey=mkey)
...
def fortios_firewall_ipmacbinding(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_ipmacbinding_table')
    if data['firewall_ipmacbinding_table']:
        resp = firewall_ipmacbinding_table(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_ipmacbinding_table'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_ipmacbinding_table"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ipmacbinding_table"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_ipmacbinding_table")

        is_error, has_changed, result = fortios_firewall_ipmacbinding(module.params, fos, module.check_mode)
