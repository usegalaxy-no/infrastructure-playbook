...
def filter_system_dhcp6_server_data(json):
    option_list = ['dns_search_list', 'dns_server1', 'dns_server2',
                   'dns_server3', 'dns_server4', 'dns_service',
                   'domain', 'id', 'interface',
                   'ip_mode', 'ip_range', 'lease_time',
                   'option1', 'option2', 'option3',
                   'prefix_mode', 'prefix_range', 'rapid_commit',
                   'status', 'subnet', 'upstream_interface']
    dictionary = {}
...
def system_dhcp6_server(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system_dhcp6', 'server', filtered_data, vdom=vdom)
        current_data = fos.get('system_dhcp6', 'server', vdom=vdom, mkey=mkey)
...
def fortios_system_dhcp6(data, fos, check_mode):
    ...
    fos.do_member_operation('system_dhcp6_server')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_dhcp6_server"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_dhcp6_server")

        is_error, has_changed, result = fortios_system_dhcp6(module.params, fos, module.check_mode)
