...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_log_fortianalyzer2_override_filter
...
def filter_log_fortianalyzer2_override_filter_data(json):
    option_list = ['anomaly', 'dlp_archive', 'filter',
                   'filter_type', 'forward_traffic', 'free_style',
                   'gtp', 'local_traffic', 'multicast_traffic',
                   'severity', 'sniffer_traffic', 'voip']
    dictionary = {}
...
def fortios_log_fortianalyzer2(data, fos):
    ...
    fos.do_member_operation('log_fortianalyzer2_override_filter')
    if data['log_fortianalyzer2_override_filter']:
        resp = log_fortianalyzer2_override_filter(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('log_fortianalyzer2_override_filter'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["log_fortianalyzer2_override_filter"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["log_fortianalyzer2_override_filter"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "log_fortianalyzer2_override_filter")
