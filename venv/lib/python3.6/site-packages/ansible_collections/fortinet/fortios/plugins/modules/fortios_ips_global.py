...
def filter_ips_global_data(json):
    option_list = ['anomaly_mode', 'cp_accel_mode', 'database',
                   'deep_app_insp_db_limit', 'deep_app_insp_timeout', 'engine_count',
                   'exclude_signatures', 'fail_open', 'intelligent_mode',
                   'ips_reserve_cpu', 'ngfw_max_scan_range', 'np_accel_mode',
                   'packet_log_queue_depth', 'session_limit_mode', 'skype_client_public_ipaddr',
                   'socket_size', 'sync_session_ttl', 'tls_active_probe',
                   'traffic_submit']
    dictionary = {}
...
def fortios_ips(data, fos):
    ...
    fos.do_member_operation('ips_global')
    if data['ips_global']:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["ips_global"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "ips_global")
