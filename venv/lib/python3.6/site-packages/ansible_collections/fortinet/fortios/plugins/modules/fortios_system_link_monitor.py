...
def filter_system_link_monitor_data(json):
    option_list = ['addr_mode', 'class_id', 'diffservcode',
                   'fail_weight', 'failtime', 'gateway_ip',
                   'gateway_ip6', 'ha_priority', 'http_agent',
                   'http_get', 'http_match', 'interval',
                   'name', 'packet_size', 'password',
                   'port', 'probe_count', 'probe_timeout',
                   'protocol', 'recoverytime', 'route',
                   'security_mode', 'server', 'server_config',
                   'server_list', 'service_detection', 'source_ip',
...
def system_link_monitor(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'link_monitor', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'link_monitor', vdom=vdom, mkey=mkey)
