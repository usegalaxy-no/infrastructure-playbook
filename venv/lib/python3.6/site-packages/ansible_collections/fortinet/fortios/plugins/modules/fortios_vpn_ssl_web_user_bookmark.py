...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_vpn_ssl_web_user_bookmark
...
def vpn_ssl_web_user_bookmark(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_ssl_web', 'user_bookmark', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_ssl_web', 'user_bookmark', vdom=vdom, mkey=mkey)
...
def fortios_vpn_ssl_web(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_ssl_web_user_bookmark')
    if data['vpn_ssl_web_user_bookmark']:
        resp = vpn_ssl_web_user_bookmark(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('vpn_ssl_web_user_bookmark'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_ssl_web_user_bookmark"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ssl_web_user_bookmark"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_ssl_web_user_bookmark")
