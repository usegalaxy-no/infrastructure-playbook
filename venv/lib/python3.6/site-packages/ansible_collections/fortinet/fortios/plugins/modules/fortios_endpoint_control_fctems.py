...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_endpoint_control_fctems
    fortios_endpoint_control_fctems:
      ...
      endpoint_control_fctems:
        ...
        fortinetone_cloud_authentication: disable
        https_port: 443
        name: terr-epc-fctems
        pull_avatars: enable
        pull_sysinfo: enable
        pull_tags: enable
...
def filter_endpoint_control_fctems_data(json):
    option_list = ['admin_password', 'admin_username', 'call_timeout',
                   ...
                   'preserve_ssl_session', 'pull_avatars', 'pull_malware_hash',
                   'pull_sysinfo', 'pull_tags', 'pull_vulnerabilities',
                   'serial_number', 'server', 'source_ip',
                   'status_check_interval', 'websocket_override']
    dictionary = {}
...
def endpoint_control_fctems(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('endpoint_control', 'fctems', filtered_data, vdom=vdom)
        current_data = fos.get('endpoint_control', 'fctems', vdom=vdom, mkey=mkey)
...
def fortios_endpoint_control(data, fos, check_mode):
    ...
    fos.do_member_operation('endpoint_control_fctems')
