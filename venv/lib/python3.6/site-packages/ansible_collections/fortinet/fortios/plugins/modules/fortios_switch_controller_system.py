...
def filter_switch_controller_system_data(json):
    option_list = ['data_sync_interval', 'dynamic_periodic_interval', 'iot_holdoff',
                   'iot_mac_idle', 'iot_scan_interval', 'iot_weight_threshold',
                   'nac_periodic_interval', 'parallel_process', 'parallel_process_override',
                   'tunnel_mode']
    dictionary = {}
...
def fortios_switch_controller(data, fos):
    ...
    fos.do_member_operation('switch_controller_system')
    if data['switch_controller_system']:
        resp = switch_controller_system(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('switch_controller_system'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_controller_system"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_system"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_controller_system")
