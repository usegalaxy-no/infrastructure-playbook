...
def dlp_fp_sensitivity(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('dlp', 'fp_sensitivity', filtered_data, vdom=vdom)
        current_data = fos.get('dlp', 'fp_sensitivity', vdom=vdom, mkey=mkey)
...
def fortios_dlp(data, fos, check_mode):
    ...
    fos.do_member_operation('dlp_fp_sensitivity')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["dlp_fp_sensitivity"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "dlp_fp_sensitivity")
