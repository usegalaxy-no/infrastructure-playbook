...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_firewall_internet_service_definition
...
def firewall_internet_service_definition(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'internet_service_definition', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'internet_service_definition', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_internet_service_definition')
    if data['firewall_internet_service_definition']:
        resp = firewall_internet_service_definition(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_internet_service_definition'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_internet_service_definition"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_internet_service_definition"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_internet_service_definition")
