...
def filter_firewall_multicast_policy_data(json):
    option_list = ['action', 'auto_asic_offload', 'comments',
                   'dnat', 'dstaddr', 'dstintf',
                   'end_port', 'id', 'logtraffic',
                   'name', 'protocol', 'snat',
                   'snat_ip', 'srcaddr', 'srcintf',
                   'start_port', 'status', 'uuid']
    dictionary = {}
...
def firewall_multicast_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'multicast_policy', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'multicast_policy', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_multicast_policy')
    if data['firewall_multicast_policy']:
        resp = firewall_multicast_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_multicast_policy'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_multicast_policy"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_multicast_policy"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_multicast_policy")
