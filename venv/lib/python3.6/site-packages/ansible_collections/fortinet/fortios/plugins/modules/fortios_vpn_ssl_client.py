...
short_description: clien in Fortinet's FortiOS and FortiGate.
description:
...
def filter_vpn_ssl_client_data(json):
    option_list = ['certificate', 'comment', 'distance',
                   'interface', 'name', 'peer',
                   'port', 'priority', 'psk',
                   'realm', 'server', 'source_ip',
                   'status', 'user']
    dictionary = {}
...
def fortios_vpn_ssl(data, fos):
    ...
    fos.do_member_operation('vpn_ssl_client')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_ssl_client"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_ssl_client")
