...
def filter_firewall_shaper_per_ip_shaper_data(json):
    option_list = ['bandwidth_unit', 'diffserv_forward', 'diffserv_reverse',
                   ...
                   'max_concurrent_session', 'max_concurrent_tcp_session', 'max_concurrent_udp_session',
                   'name']
    dictionary = {}
...
def firewall_shaper_per_ip_shaper(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_shaper', 'per_ip_shaper', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_shaper', 'per_ip_shaper', vdom=vdom, mkey=mkey)
...
def fortios_firewall_shaper(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_shaper_per_ip_shaper')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_shaper_per_ip_shaper"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_shaper_per_ip_shaper"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_shaper_per_ip_shaper")

        is_error, has_changed, result = fortios_firewall_shaper(module.params, fos, module.check_mode)
