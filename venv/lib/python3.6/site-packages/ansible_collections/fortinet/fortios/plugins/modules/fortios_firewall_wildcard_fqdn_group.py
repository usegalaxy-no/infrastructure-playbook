...
def firewall_wildcard_fqdn_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_wildcard_fqdn', 'group', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_wildcard_fqdn', 'group', vdom=vdom, mkey=mkey)
...
def fortios_firewall_wildcard_fqdn(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_wildcard_fqdn_group')
    if data['firewall_wildcard_fqdn_group']:
        resp = firewall_wildcard_fqdn_group(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_wildcard_fqdn_group'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_wildcard_fqdn_group"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_wildcard_fqdn_group"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_wildcard_fqdn_group")
