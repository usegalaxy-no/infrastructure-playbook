...
def firewall_shaper_traffic_shaper(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_shaper', 'traffic_shaper', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_shaper', 'traffic_shaper', vdom=vdom, mkey=mkey)
...
def fortios_firewall_shaper(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_shaper_traffic_shaper')
