...
def filter_system_switch_interface_data(json):
    option_list = ['intra_switch_policy', 'mac_ttl', 'member',
                   'name', 'span', 'span_dest_port',
                   'span_direction', 'span_source_port', 'type',
                   'vdom']
    dictionary = {}
...
def system_switch_interface(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'switch_interface', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'switch_interface', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_switch_interface')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_switch_interface"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_switch_interface"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_switch_interface")
