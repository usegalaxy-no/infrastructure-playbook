...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_system_lldp_network_policy
...
def system_lldp_network_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system_lldp', 'network_policy', filtered_data, vdom=vdom)
        current_data = fos.get('system_lldp', 'network_policy', vdom=vdom, mkey=mkey)
...
def fortios_system_lldp(data, fos, check_mode):
    ...
    fos.do_member_operation('system_lldp_network_policy')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_lldp_network_policy"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_lldp_network_policy"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_lldp_network_policy")

        is_error, has_changed, result = fortios_system_lldp(module.params, fos, module.check_mode)
