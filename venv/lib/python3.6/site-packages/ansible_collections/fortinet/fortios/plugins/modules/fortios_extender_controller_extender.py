...
def flatten_multilists_attributes(data):
    multilist_attrs = [
        [u'modem1', u'auto_switch', u'switch_back'],
        [u'modem2', u'auto_switch', u'switch_back'],
    ]
...
def extender_controller_extender(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('extender_controller', 'extender', filtered_data, vdom=vdom)
        current_data = fos.get('extender_controller', 'extender', vdom=vdom, mkey=mkey)
...
def fortios_extender_controller(data, fos, check_mode):
    ...
    fos.do_member_operation('extender_controller_extender')
