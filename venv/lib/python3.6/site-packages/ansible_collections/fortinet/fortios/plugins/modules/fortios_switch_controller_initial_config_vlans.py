...
options:
    ...
    switch_controller_initial_config_vlans:
        ...
        suboptions:
            ...
            nac_segment:
                description:
                    - VLAN for NAC segemnt primary interface. Source switch-controller.initial-config.template.name.
                type: str
            quarantine:
                description:
...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_switch_controller_initial_config_vlans
...
def filter_switch_controller_initial_config_vlans_data(json):
    option_list = ['default_vlan', 'nac', 'nac_segment',
                   'quarantine', 'rspan', 'video',
                   'voice']
    dictionary = {}
...
def switch_controller_initial_config_vlans(data, fos):
    vdom = data['vdom']
    switch_controller_initial_config_vlans_data = data['switch_controller_initial_config_vlans']
    filtered_data = underscore_to_hyphen(filter_switch_controller_initial_config_vlans_data(switch_controller_initial_config_vlans_data))
...
def fortios_switch_controller_initial_config(data, fos):
    ...
    fos.do_member_operation('switch_controller_initial_config_vlans')
    if data['switch_controller_initial_config_vlans']:
        resp = switch_controller_initial_config_vlans(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('switch_controller_initial_config_vlans'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_controller_initial_config_vlans"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_initial_config_vlans"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_controller_initial_config_vlans")

        is_error, has_changed, result = fortios_switch_controller_initial_config(module.params, fos)
