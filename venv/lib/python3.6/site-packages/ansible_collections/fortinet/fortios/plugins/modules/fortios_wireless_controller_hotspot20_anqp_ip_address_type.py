...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_wireless_controller_hotspot20_anqp_ip_address_type
...
def wireless_controller_hotspot20_anqp_ip_address_type(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('wireless_controller_hotspot20', 'anqp_ip_address_type', filtered_data, vdom=vdom)
        current_data = fos.get('wireless_controller_hotspot20', 'anqp_ip_address_type', vdom=vdom, mkey=mkey)
...
def fortios_wireless_controller_hotspot20(data, fos, check_mode):
    ...
    fos.do_member_operation('wireless_controller_hotspot20_anqp_ip_address_type')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["wireless_controller_hotspot20_anqp_ip_address_type"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["wireless_controller_hotspot20_anqp_ip_address_type"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "wireless_controller_hotspot20_anqp_ip_address_type")
