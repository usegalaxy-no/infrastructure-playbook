...
def web_proxy_url_match(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('web_proxy', 'url_match', filtered_data, vdom=vdom)
        current_data = fos.get('web_proxy', 'url_match', vdom=vdom, mkey=mkey)
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    fos.do_member_operation('web_proxy_url_match')
