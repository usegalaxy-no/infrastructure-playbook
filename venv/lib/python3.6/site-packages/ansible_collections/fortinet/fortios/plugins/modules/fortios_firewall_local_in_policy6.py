...
def filter_firewall_local_in_policy6_data(json):
    option_list = ['action', 'comments', 'dstaddr',
                   'dstaddr_negate', 'intf', 'policyid',
                   'schedule', 'service', 'service_negate',
                   'srcaddr', 'srcaddr_negate', 'status',
                   'uuid']
    dictionary = {}
...
def firewall_local_in_policy6(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'local_in_policy6', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'local_in_policy6', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_local_in_policy6')
    if data['firewall_local_in_policy6']:
        resp = firewall_local_in_policy6(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_local_in_policy6'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_local_in_policy6"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_local_in_policy6"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_local_in_policy6")
