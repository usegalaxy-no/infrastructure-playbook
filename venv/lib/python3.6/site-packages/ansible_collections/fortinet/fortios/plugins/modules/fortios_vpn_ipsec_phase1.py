...
def filter_vpn_ipsec_phase1_data(json):
    option_list = ['acct_verify', 'add_gw_route', 'add_route',
                   ...
                   'interface', 'ip_delay_interval', 'ipv4_dns_server1',
...
def vpn_ipsec_phase1(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_ipsec', 'phase1', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_ipsec', 'phase1', vdom=vdom, mkey=mkey)
...
def fortios_vpn_ipsec(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_ipsec_phase1')
