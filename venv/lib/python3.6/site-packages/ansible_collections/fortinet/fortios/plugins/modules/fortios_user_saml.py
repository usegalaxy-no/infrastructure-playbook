...
def filter_user_saml_data(json):
    option_list = ['adfs_claim', 'cert', 'digest_method',
                   'entity_id', 'group_claim_type', 'group_name',
                   'idp_cert', 'idp_entity_id', 'idp_single_logout_url',
                   'idp_single_sign_on_url', 'limit_relaystate', 'name',
                   'single_logout_url', 'single_sign_on_url', 'user_claim_type',
                   'user_name']
    dictionary = {}
...
def fortios_user(data, fos, check_mode):
    ...
    fos.do_member_operation('user_saml')
    if data['user_saml']:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["user_saml"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "user_saml")
