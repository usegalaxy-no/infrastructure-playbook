...
def filter_ftp_proxy_explicit_data(json):
    option_list = ['incoming_ip', 'incoming_port', 'outgoing_ip',
                   'sec_default_action', 'ssl', 'ssl_algorithm',
                   'ssl_cert', 'ssl_dh_bits', 'status']
    dictionary = {}
...
def fortios_ftp_proxy(data, fos):
    ...
    fos.do_member_operation('ftp_proxy_explicit')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["ftp_proxy_explicit"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "ftp_proxy_explicit")
