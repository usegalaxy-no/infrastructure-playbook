...
def dnsfilter_domain_filter(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('dnsfilter', 'domain_filter', filtered_data, vdom=vdom)
        current_data = fos.get('dnsfilter', 'domain_filter', vdom=vdom, mkey=mkey)
...
def fortios_dnsfilter(data, fos, check_mode):
    ...
    fos.do_member_operation('dnsfilter_domain_filter')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["dnsfilter_domain_filter"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["dnsfilter_domain_filter"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "dnsfilter_domain_filter")
