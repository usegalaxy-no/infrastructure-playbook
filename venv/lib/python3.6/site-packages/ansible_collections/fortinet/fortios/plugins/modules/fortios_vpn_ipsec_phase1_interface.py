...
def filter_vpn_ipsec_phase1_interface_data(json):
    option_list = ['acct_verify', 'add_gw_route', 'add_route',
                   ...
                   'interface', 'ip_delay_interval', 'ip_fragmentation',
                   'ip_version', 'ipv4_dns_server1', 'ipv4_dns_server2',
                   ...
                   'keylife', 'local_gw', 'local_gw6',
                   'localid', 'localid_type', 'loopback_asymroute',
                   ...
                   'network_id', 'network_overlay', 'npu_offload',
                   'passive_mode', 'peer', 'peergrp',
                   'peerid', 'peertype', 'ppk',
                   'ppk_identity', 'ppk_secret', 'priority',
                   'proposal', 'psksecret', 'psksecret_remote',
                   'reauth', 'rekey', 'remote_gw',
                   'remote_gw6', 'remotegw_ddns', 'rsa_signature_format',
                   'save_password', 'send_cert_chain', 'signature_hash_alg',
                   'split_include_service', 'suite_b', 'tunnel_search',
                   'type', 'unity_support', 'usrgrp',
                   'vni', 'wizard_type', 'xauthtype']
    dictionary = {}
...
def vpn_ipsec_phase1_interface(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_ipsec', 'phase1_interface', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_ipsec', 'phase1_interface', vdom=vdom, mkey=mkey)
...
def fortios_vpn_ipsec(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_ipsec_phase1_interface')
