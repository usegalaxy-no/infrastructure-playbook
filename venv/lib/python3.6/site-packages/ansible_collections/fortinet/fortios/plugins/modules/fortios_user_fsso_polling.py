...
def user_fsso_polling(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('user', 'fsso_polling', filtered_data, vdom=vdom)
        current_data = fos.get('user', 'fsso_polling', vdom=vdom, mkey=mkey)
...
def fortios_user(data, fos, check_mode):
    ...
    fos.do_member_operation('user_fsso_polling')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["user_fsso_polling"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "user_fsso_polling")
