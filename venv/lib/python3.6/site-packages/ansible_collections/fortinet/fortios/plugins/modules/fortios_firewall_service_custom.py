...
def filter_firewall_service_custom_data(json):
    option_list = ['app_category', 'app_service_type', 'application',
                   ...
                   'comment', 'fabric_object', 'fqdn',
                   'helper', 'icmpcode', 'icmptype',
                   'iprange', 'name', 'protocol',
                   'protocol_number', 'proxy', 'sctp_portrange',
                   'session_ttl', 'tcp_halfclose_timer', 'tcp_halfopen_timer',
                   'tcp_portrange', 'tcp_rst_timer', 'tcp_timewait_timer',
                   'udp_idle_timer', 'udp_portrange', 'visibility']
    dictionary = {}
...
def firewall_service_custom(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_service', 'custom', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_service', 'custom', vdom=vdom, mkey=mkey)
...
def fortios_firewall_service(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_service_custom')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_service_custom"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_service_custom"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_service_custom")

        is_error, has_changed, result = fortios_firewall_service(module.params, fos, module.check_mode)
