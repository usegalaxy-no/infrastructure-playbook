...
def firewall_ssl_server(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'ssl_server', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'ssl_server', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_ssl_server')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_ssl_server"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_ssl_server")
