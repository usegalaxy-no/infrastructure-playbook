...
def filter_system_3g_modem_custom_data(json):
    option_list = ['class_id', 'id', 'init_string',
                   'model', 'modeswitch_string', 'product_id',
                   'vendor', 'vendor_id']
    dictionary = {}
...
def system_3g_modem_custom(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system_3g_modem', 'custom', filtered_data, vdom=vdom)
        current_data = fos.get('system_3g_modem', 'custom', vdom=vdom, mkey=mkey)
...
def fortios_system_3g_modem(data, fos, check_mode):
    ...
    fos.do_member_operation('system_3g_modem_custom')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_3g_modem_custom"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_3g_modem_custom"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_3g_modem_custom")

        is_error, has_changed, result = fortios_system_3g_modem(module.params, fos, module.check_mode)
