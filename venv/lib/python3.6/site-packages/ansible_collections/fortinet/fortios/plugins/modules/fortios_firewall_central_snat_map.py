...
def filter_firewall_central_snat_map_data(json):
    option_list = ['comments', 'dst_addr', 'dst_addr6',
                   'dstintf', 'nat', 'nat_ippool',
                   'nat_ippool6', 'nat_port', 'nat46',
                   'nat64', 'orig_addr', 'orig_addr6',
                   'orig_port', 'policyid', 'protocol',
                   'srcintf', 'status', 'type',
                   'uuid']
    dictionary = {}
...
def firewall_central_snat_map(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'central_snat_map', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'central_snat_map', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_central_snat_map')
    if data['action'] == 'move':
        resp = move_fortios_firewall(data, fos)
    elif data['firewall_central_snat_map']:
        resp = firewall_central_snat_map(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_central_snat_map'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_central_snat_map"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_central_snat_map"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_central_snat_map")
