...
def endpoint_control_client(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('endpoint_control', 'client', filtered_data, vdom=vdom)
        current_data = fos.get('endpoint_control', 'client', vdom=vdom, mkey=mkey)
...
def fortios_endpoint_control(data, fos, check_mode):
    ...
    fos.do_member_operation('endpoint_control_client')
