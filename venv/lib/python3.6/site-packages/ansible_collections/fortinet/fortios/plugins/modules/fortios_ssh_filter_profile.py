...
def flatten_multilists_attributes(data):
    multilist_attrs = [
        [u'file_filter', u'entries', u'protocol'],
        [u'block'],
        [u'log'],
    ]
...
def ssh_filter_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('ssh_filter', 'profile', filtered_data, vdom=vdom)
        current_data = fos.get('ssh_filter', 'profile', vdom=vdom, mkey=mkey)
...
def fortios_ssh_filter(data, fos, check_mode):
    ...
    fos.do_member_operation('ssh_filter_profile')
