...
def extender_controller_dataplan(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('extender_controller', 'dataplan', filtered_data, vdom=vdom)
        current_data = fos.get('extender_controller', 'dataplan', vdom=vdom, mkey=mkey)
...
def fortios_extender_controller(data, fos, check_mode):
    ...
    fos.do_member_operation('extender_controller_dataplan')
