...
def webfilter_search_engine(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('webfilter', 'search_engine', filtered_data, vdom=vdom)
        current_data = fos.get('webfilter', 'search_engine', vdom=vdom, mkey=mkey)
...
def fortios_webfilter(data, fos, check_mode):
    ...
    fos.do_member_operation('webfilter_search_engine')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["webfilter_search_engine"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["webfilter_search_engine"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "webfilter_search_engine")
