...
def filter_firewall_ldb_monitor_data(json):
    option_list = ['dns_match_ip', 'dns_protocol', 'dns_request_domain',
                   'http_get', 'http_match', 'http_max_redirects',
                   'interval', 'name', 'port',
                   'retry', 'src_ip', 'timeout',
                   'type']
    dictionary = {}
...
def firewall_ldb_monitor(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'ldb_monitor', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'ldb_monitor', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_ldb_monitor')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_ldb_monitor"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_ldb_monitor")
