...
def vpn_ipsec_manualkey_interface(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_ipsec', 'manualkey_interface', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_ipsec', 'manualkey_interface', vdom=vdom, mkey=mkey)
...
def fortios_vpn_ipsec(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_ipsec_manualkey_interface')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_ipsec_manualkey_interface"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_ipsec_manualkey_interface"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_ipsec_manualkey_interface")
