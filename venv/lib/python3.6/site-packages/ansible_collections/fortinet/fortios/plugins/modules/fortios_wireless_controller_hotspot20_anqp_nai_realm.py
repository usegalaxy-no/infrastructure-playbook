...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_wireless_controller_hotspot20_anqp_nai_realm
...
def wireless_controller_hotspot20_anqp_nai_realm(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('wireless_controller_hotspot20', 'anqp_nai_realm', filtered_data, vdom=vdom)
        current_data = fos.get('wireless_controller_hotspot20', 'anqp_nai_realm', vdom=vdom, mkey=mkey)
...
def fortios_wireless_controller_hotspot20(data, fos, check_mode):
    ...
    fos.do_member_operation('wireless_controller_hotspot20_anqp_nai_realm')
    if data['wireless_controller_hotspot20_anqp_nai_realm']:
        ...
        fos._module.fail_json(msg='missing task body: %s' % ('wireless_controller_hotspot20_anqp_nai_realm'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["wireless_controller_hotspot20_anqp_nai_realm"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["wireless_controller_hotspot20_anqp_nai_realm"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "wireless_controller_hotspot20_anqp_nai_realm")
