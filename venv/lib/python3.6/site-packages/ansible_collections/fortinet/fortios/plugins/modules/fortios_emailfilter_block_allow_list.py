...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_emailfilter_block_allow_list
    fortios_emailfilter_block_allow_list:
      vdom: root
      state: present
      emailfilter_block_allow_list:
        id: 1
        name: terr-emfilter-blk
...
def emailfilter_block_allow_list(data, fos):
    ...
    filtered_data = underscore_to_hyphen(filter_emailfilter_block_allow_list_data(emailfilter_block_allow_list_data))
...
def fortios_emailfilter(data, fos):
    ...
    fos.do_member_operation('emailfilter_block_allow_list')
    if data['emailfilter_block_allow_list']:
        resp = emailfilter_block_allow_list(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('emailfilter_block_allow_list'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["emailfilter_block_allow_list"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["emailfilter_block_allow_list"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "emailfilter_block_allow_list")
