...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_switch_controller_qos_queue_policy
...
def switch_controller_qos_queue_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('switch_controller_qos', 'queue_policy', filtered_data, vdom=vdom)
        current_data = fos.get('switch_controller_qos', 'queue_policy', vdom=vdom, mkey=mkey)
...
def fortios_switch_controller_qos(data, fos, check_mode):
    ...
    fos.do_member_operation('switch_controller_qos_queue_policy')
    if data['switch_controller_qos_queue_policy']:
        resp = switch_controller_qos_queue_policy(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('switch_controller_qos_queue_policy'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_controller_qos_queue_policy"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_qos_queue_policy"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_controller_qos_queue_policy")
