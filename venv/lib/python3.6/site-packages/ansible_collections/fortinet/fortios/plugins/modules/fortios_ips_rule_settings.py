...
def ips_rule_settings(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('ips', 'rule_settings', filtered_data, vdom=vdom)
        current_data = fos.get('ips', 'rule_settings', vdom=vdom, mkey=mkey)
...
def fortios_ips(data, fos, check_mode):
    ...
    fos.do_member_operation('ips_rule_settings')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["ips_rule_settings"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "ips_rule_settings")
