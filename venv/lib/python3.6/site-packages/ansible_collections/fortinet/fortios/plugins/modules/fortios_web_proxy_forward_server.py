...
def web_proxy_forward_server(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('web_proxy', 'forward_server', filtered_data, vdom=vdom)
        current_data = fos.get('web_proxy', 'forward_server', vdom=vdom, mkey=mkey)
...
def fortios_web_proxy(data, fos, check_mode):
    ...
    fos.do_member_operation('web_proxy_forward_server')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["web_proxy_forward_server"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["web_proxy_forward_server"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "web_proxy_forward_server")
