...
def filter_firewall_profile_group_data(json):
    option_list = ['application_list', 'av_profile', 'cifs_profile',
                   ...
                   'file_filter_profile', 'icap_profile', 'ips_sensor',
                   'mms_profile', 'name', 'profile_protocol_options',
                   'sctp_filter_profile', 'spamfilter_profile', 'ssh_filter_profile',
                   'ssl_ssh_profile', 'videofilter_profile', 'voip_profile',
...
def firewall_profile_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'profile_group', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'profile_group', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_profile_group')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_profile_group"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_profile_group"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_profile_group")
