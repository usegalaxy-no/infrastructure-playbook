...
def filter_firewall_schedule_group_data(json):
    option_list = ['color', 'fabric_object', 'member',
                   'name']
    dictionary = {}
...
def firewall_schedule_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall_schedule', 'group', filtered_data, vdom=vdom)
        current_data = fos.get('firewall_schedule', 'group', vdom=vdom, mkey=mkey)
...
def fortios_firewall_schedule(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_schedule_group')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_schedule_group"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_schedule_group"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_schedule_group")
