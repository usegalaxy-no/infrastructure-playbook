...
def filter_user_domain_controller_data(json):
    option_list = ['ad_mode', 'adlds_dn', 'adlds_ip_address',
                   'adlds_ip6', 'adlds_port', 'dns_srv_lookup',
                   'domain_name', 'extra_server', 'hostname',
                   'interface', 'interface_select_method', 'ip_address',
                   'ip6', 'ldap_server', 'name',
                   'password', 'port', 'replication_port',
                   'source_ip_address', 'source_ip6', 'source_port',
                   'username']
    dictionary = {}
...
def user_domain_controller(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('user', 'domain_controller', filtered_data, vdom=vdom)
        current_data = fos.get('user', 'domain_controller', vdom=vdom, mkey=mkey)
...
def fortios_user(data, fos, check_mode):
    ...
    fos.do_member_operation('user_domain_controller')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["user_domain_controller"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["user_domain_controller"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "user_domain_controller")
