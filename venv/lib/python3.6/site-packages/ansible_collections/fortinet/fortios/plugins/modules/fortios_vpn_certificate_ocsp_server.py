...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_vpn_certificate_ocsp_server
...
def vpn_certificate_ocsp_server(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_certificate', 'ocsp_server', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_certificate', 'ocsp_server', vdom=vdom, mkey=mkey)
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_certificate_ocsp_server')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_certificate_ocsp_server"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_certificate_ocsp_server"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_certificate_ocsp_server")
