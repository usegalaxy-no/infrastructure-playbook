...
def user_device_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('user', 'device_group', filtered_data, vdom=vdom)
        current_data = fos.get('user', 'device_group', vdom=vdom, mkey=mkey)
...
def fortios_user(data, fos, check_mode):
    ...
    fos.do_member_operation('user_device_group')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["user_device_group"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "user_device_group")
