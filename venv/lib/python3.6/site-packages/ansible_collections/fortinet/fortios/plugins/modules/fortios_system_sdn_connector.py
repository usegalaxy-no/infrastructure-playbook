...
def filter_system_sdn_connector_data(json):
    option_list = ['access_key', 'api_key', 'azure_region',
                   'client_id', 'client_secret', 'compartment_id',
                   'compute_generation', 'domain', 'external_ip',
                   'gcp_project', 'group_name', 'ha_status',
                   'ibm_region', 'ibm_region_gen1', 'ibm_region_gen2',
                   ...
                   'server_list', 'server_port', 'service_account',
                   'status', 'subscription_id', 'tenant_id',
                   'type', 'update_interval', 'use_metadata_iam',
                   'user_id', 'username', 'vcenter_password',
                   'vcenter_server', 'vcenter_username', 'verify_certificate',
                   'vpc_id']
    dictionary = {}
...
def system_sdn_connector(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'sdn_connector', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'sdn_connector', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_sdn_connector')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_sdn_connector"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_sdn_connector")
