    ...
    fortios_endpoint_control_profile:
      ...
      endpoint_control_profile:
        ...
        - av_realtime_protection: disable
          ...
          forticlient_log_upload_level: event
...
def endpoint_control_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('endpoint_control', 'profile', filtered_data, vdom=vdom)
        current_data = fos.get('endpoint_control', 'profile', vdom=vdom, mkey=mkey)
...
def fortios_endpoint_control(data, fos, check_mode):
    ...
    fos.do_member_operation('endpoint_control_profile')
    if data['endpoint_control_profile']:
        resp = endpoint_control_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('endpoint_control_profile'))
    if check_mode:
        return resp
