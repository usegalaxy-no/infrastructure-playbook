...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_system_replacemsg_spam
    fortios_system_replacemsg_spam:
      vdom: root
      state: present
      system_replacemsg_spam:
        buffer: Mail from this IP address is not allowed and has been blocked.
        format: text
        header: none
        msg_type: ipblocklist
...
def system_replacemsg_spam(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system_replacemsg', 'spam', filtered_data, vdom=vdom)
        current_data = fos.get('system_replacemsg', 'spam', vdom=vdom, mkey=mkey)
...
def fortios_system_replacemsg(data, fos, check_mode):
    ...
    fos.do_member_operation('system_replacemsg_spam')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_replacemsg_spam"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_spam"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_replacemsg_spam")
