...
def filter_vpn_certificate_local_data(json):
    option_list = ['acme_ca_url', 'acme_domain', 'acme_email',
                   'acme_renew_window', 'acme_rsa_key_size', 'auto_regenerate_days',
                   'auto_regenerate_days_warning', 'ca_identifier', 'certificate',
                   'cmp_path', 'cmp_regeneration_method', 'cmp_server',
                   'cmp_server_cert', 'comments', 'csr',
                   'enroll_protocol', 'ike_localid', 'ike_localid_type',
                   'last_updated', 'name', 'name_encoding',
                   'password', 'private_key', 'range',
                   'scep_password', 'scep_url', 'source',
                   'source_ip', 'state']
    dictionary = {}
...
def vpn_certificate_local(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_certificate', 'local', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_certificate', 'local', vdom=vdom, mkey=mkey)
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_certificate_local')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_certificate_local"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["vpn_certificate_local"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_certificate_local")

        is_error, has_changed, result = fortios_vpn_certificate(module.params, fos, module.check_mode)
