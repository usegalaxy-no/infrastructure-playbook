...
def application_rule_settings(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('application', 'rule_settings', filtered_data, vdom=vdom)
        current_data = fos.get('application', 'rule_settings', vdom=vdom, mkey=mkey)
...
def fortios_application(data, fos, check_mode):
    ...
    fos.do_member_operation('application_rule_settings')
    if data['application_rule_settings']:
        resp = application_rule_settings(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('application_rule_settings'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["application_rule_settings"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["application_rule_settings"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "application_rule_settings")
