...
def filter_vpn_certificate_ca_data(json):
    option_list = ['auto_update_days', 'auto_update_days_warning', 'ca',
                   ...
                   'ssl_inspection_trusted', 'trusted']
    dictionary = {}
...
def vpn_certificate_ca(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_certificate', 'ca', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_certificate', 'ca', vdom=vdom, mkey=mkey)
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_certificate_ca')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_certificate_ca"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_certificate_ca")
