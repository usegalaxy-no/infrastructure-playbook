...
def wanopt_content_delivery_network_rule(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('wanopt', 'content_delivery_network_rule', filtered_data, vdom=vdom)
        current_data = fos.get('wanopt', 'content_delivery_network_rule', vdom=vdom, mkey=mkey)
...
def fortios_wanopt(data, fos, check_mode):
    ...
    fos.do_member_operation('wanopt_content_delivery_network_rule')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["wanopt_content_delivery_network_rule"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["wanopt_content_delivery_network_rule"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "wanopt_content_delivery_network_rule")

        is_error, has_changed, result = fortios_wanopt(module.params, fos, module.check_mode)
