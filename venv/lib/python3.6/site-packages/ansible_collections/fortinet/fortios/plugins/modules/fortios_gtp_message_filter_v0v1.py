...
def filter_gtp_message_filter_v0v1_data(json):
    option_list = ['create_mbms', 'create_pdp', 'data_record',
                   'delete_aa_pdp', 'delete_mbms', 'delete_pdp',
                   'echo', 'end_marker', 'error_indication',
                   'failure_report', 'fwd_relocation', 'fwd_srns_context',
                   'gtp_pdu', 'identification', 'mbms_de_registration',
                   'mbms_notification', 'mbms_registration', 'mbms_session_start',
                   'mbms_session_stop', 'mbms_session_update', 'ms_info_change_notif',
                   'name', 'node_alive', 'note_ms_present',
                   'pdu_notification', 'ran_info', 'redirection',
                   'relocation_cancel', 'send_route', 'sgsn_context',
                   'support_extension', 'unknown_message', 'unknown_message_white_list',
                   'update_mbms', 'update_pdp', 'v0_create_aa_pdp__v1_init_pdp_ctx',
                   'version_not_support']
    dictionary = {}
...
def gtp_message_filter_v0v1(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_gtp_message_filter_v0v1_data(gtp_message_filter_v0v1_data))

    # check_mode starts from here
    if check_mode:
        mkey = fos.get_mkey('gtp', 'message_filter_v0v1', filtered_data, vdom=vdom)
        current_data = fos.get('gtp', 'message_filter_v0v1', vdom=vdom, mkey=mkey)
...
def fortios_gtp(data, fos, check_mode):
    ...
    fos.do_member_operation('gtp_message_filter_v0v1')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["gtp_message_filter_v0v1"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["gtp_message_filter_v0v1"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "gtp_message_filter_v0v1")
