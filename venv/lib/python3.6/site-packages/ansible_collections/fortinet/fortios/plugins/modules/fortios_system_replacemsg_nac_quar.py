...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_system_replacemsg_nac_quar
    fortios_system_replacemsg_nac_quar:
      vdom: root
      state: present
      system_replacemsg_nac_quar:
        buffer: <html><head><title>Administrative Quarantine</title></head><body><font
          size=2><table width="100%"><tr><td bgcolor=#3300cc align="center" colspan=2><font
          color=#ffffff><b>Blocked because of admin action</b></font></td></tr></table><br><br>Your
          system administrator has blocked your computer or device. Please contact
          the system administrator.<br><br><hr></font></body></html>
        format: html
        header: http
        msg_type: nac-quar-admin
...
def system_replacemsg_nac_quar(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system_replacemsg', 'nac_quar', filtered_data, vdom=vdom)
        current_data = fos.get('system_replacemsg', 'nac_quar', vdom=vdom, mkey=mkey)
...
def fortios_system_replacemsg(data, fos, check_mode):
    ...
    fos.do_member_operation('system_replacemsg_nac_quar')
    if data['system_replacemsg_nac_quar']:
        resp = system_replacemsg_nac_quar(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('system_replacemsg_nac_quar'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_replacemsg_nac_quar"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_nac_quar"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_replacemsg_nac_quar")
