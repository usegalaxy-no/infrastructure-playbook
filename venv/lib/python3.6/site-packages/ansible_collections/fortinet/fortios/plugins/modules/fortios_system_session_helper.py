...
def system_session_helper(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'session_helper', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'session_helper', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_session_helper')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_session_helper"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_session_helper"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_session_helper")
