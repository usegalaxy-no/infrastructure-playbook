...
def system_ips_urlfilter_dns(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'ips_urlfilter_dns', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'ips_urlfilter_dns', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_ips_urlfilter_dns')
    if data['system_ips_urlfilter_dns']:
        resp = system_ips_urlfilter_dns(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('system_ips_urlfilter_dns'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_ips_urlfilter_dns"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_ips_urlfilter_dns"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_ips_urlfilter_dns")
