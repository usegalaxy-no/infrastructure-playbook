...
def filter_system_external_resource_data(json):
    option_list = ['category', 'comments', 'interface',
                   'interface_select_method', 'name', 'password',
                   'refresh_rate', 'resource', 'source_ip',
                   'status', 'type', 'user_agent',
                   'username', 'uuid']
    dictionary = {}
...
def system_external_resource(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'external_resource', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'external_resource', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_external_resource')
    if data['system_external_resource']:
        resp = system_external_resource(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('system_external_resource'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_external_resource"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_external_resource"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_external_resource")
