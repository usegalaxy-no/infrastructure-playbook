...
def firewall_multicast_address6(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'multicast_address6', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'multicast_address6', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_multicast_address6')
    if data['firewall_multicast_address6']:
        resp = firewall_multicast_address6(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_multicast_address6'))
    if check_mode:
        return resp
