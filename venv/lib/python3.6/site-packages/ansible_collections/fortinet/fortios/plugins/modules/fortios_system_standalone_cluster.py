...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_system_standalone_cluster
    fortios_system_standalone_cluster:
      vdom: root
      system_standalone_cluster:
        encryption: disable
        group_member_id: 0
        layer2_connection: unavailable
...
def filter_system_standalone_cluster_data(json):
    option_list = ['encryption', 'group_member_id', 'layer2_connection',
                   'psksecret', 'session_sync_dev', 'standalone_group_id']
    dictionary = {}
...
def system_standalone_cluster(data, fos):
    ...
    filtered_data = underscore_to_hyphen(filter_system_standalone_cluster_data(system_standalone_cluster_data))
...
def fortios_system(data, fos):
    ...
    fos.do_member_operation('system_standalone_cluster')
    if data['system_standalone_cluster']:
        resp = system_standalone_cluster(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('system_standalone_cluster'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_standalone_cluster"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_standalone_cluster"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_standalone_cluster")
