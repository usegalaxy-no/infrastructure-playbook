...
def filter_system_dns_server_data(json):
    option_list = ['dnsfilter_profile', 'doh', 'mode',
                   'name']
    dictionary = {}
...
def system_dns_server(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'dns_server', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'dns_server', vdom=vdom, mkey=mkey)
