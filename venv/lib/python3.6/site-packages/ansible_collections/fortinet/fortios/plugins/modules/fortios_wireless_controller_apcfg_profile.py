...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_wireless_controller_apcfg_profile
...
def filter_wireless_controller_apcfg_profile_data(json):
    option_list = ['ac_ip', 'ac_port', 'ac_timer',
                   'ac_type', 'command_list', 'comment',
                   'name']
    dictionary = {}
...
def wireless_controller_apcfg_profile(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_wireless_controller_apcfg_profile_data(wireless_controller_apcfg_profile_data))

    # check_mode starts from here
    if check_mode:
        mkey = fos.get_mkey('wireless_controller', 'apcfg_profile', filtered_data, vdom=vdom)
        current_data = fos.get('wireless_controller', 'apcfg_profile', vdom=vdom, mkey=mkey)
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    fos.do_member_operation('wireless_controller_apcfg_profile')
    if data['wireless_controller_apcfg_profile']:
        resp = wireless_controller_apcfg_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('wireless_controller_apcfg_profile'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["wireless_controller_apcfg_profile"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["wireless_controller_apcfg_profile"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "wireless_controller_apcfg_profile")
