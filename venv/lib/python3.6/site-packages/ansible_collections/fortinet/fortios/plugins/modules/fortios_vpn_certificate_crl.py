...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_vpn_certificate_crl
...
def vpn_certificate_crl(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('vpn_certificate', 'crl', filtered_data, vdom=vdom)
        current_data = fos.get('vpn_certificate', 'crl', vdom=vdom, mkey=mkey)
...
def fortios_vpn_certificate(data, fos, check_mode):
    ...
    fos.do_member_operation('vpn_certificate_crl')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_certificate_crl"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_certificate_crl")
