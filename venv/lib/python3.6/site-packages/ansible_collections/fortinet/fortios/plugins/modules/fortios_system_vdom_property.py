...
- collections:
  ...
  - name: fortios_system_vdom_property
    fortios_system_vdom_property:
      ...
      system_vdom_property:
        custom_service: 0 0
        description: property-limits-for-vdom-testvdom
        dialup_tunnel: 5 0
        firewall_address: 0 0
        firewall_addrgrp: 0 0
        firewall_policy: 10 0
        ipsec_phase1: 0 0
        ipsec_phase1_interface: 0 0
        ipsec_phase2: 0 0
        ipsec_phase2_interface: 0 0
        log_disk_quota: 0 0
        name: testvdom
        onetime_schedule: 0 0
        proxy: 0 0
        recurring_schedule: 0 0
        service_group: 0 0
        session: 5 0
        snmp_index: 3
        sslvpn: 0 0
...
def system_vdom_property(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'vdom_property', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'vdom_property', vdom=vdom, mkey=mkey)
