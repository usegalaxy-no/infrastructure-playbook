...
def filter_system_replacemsg_mm4_data(json):
    option_list = ['add_smil', 'charset', 'class',
                   'domain', 'format', 'from',
                   'from_sender', 'header', 'image',
                   'message', 'msg_type', 'priority',
                   'rsp_status', 'smil_part', 'subject']
    dictionary = {}
...
def system_replacemsg_mm4(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_system_replacemsg_mm4_data(system_replacemsg_mm4_data))

    # check_mode starts from here
    if check_mode:
        mkey = fos.get_mkey('system_replacemsg', 'mm4', filtered_data, vdom=vdom)
        current_data = fos.get('system_replacemsg', 'mm4', vdom=vdom, mkey=mkey)
...
def fortios_system_replacemsg(data, fos, check_mode):
    ...
    fos.do_member_operation('system_replacemsg_mm4')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_replacemsg_mm4"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_mm4"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_replacemsg_mm4")
