...
def filter_switch_controller_security_policy_802_1x_data(json):
    option_list = ['auth_fail_vlan', 'auth_fail_vlan_id', 'auth_fail_vlanid',
                   'authserver_timeout_period', 'authserver_timeout_vlan', 'authserver_timeout_vlanid',
                   'eap_auto_untagged_vlans', 'eap_passthru', 'framevid_apply',
                   'guest_auth_delay', 'guest_vlan', 'guest_vlan_id',
                   'guest_vlanid', 'mac_auth_bypass', 'name',
                   'open_auth', 'policy_type', 'radius_timeout_overwrite',
...
def switch_controller_security_policy_802_1x(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('switch_controller_security_policy', '802_1x', filtered_data, vdom=vdom)
        current_data = fos.get('switch_controller_security_policy', '802_1x', vdom=vdom, mkey=mkey)
...
def fortios_switch_controller_security_policy(data, fos, check_mode):
    ...
    fos.do_member_operation('switch_controller_security_policy_802_1x')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["switch_controller_security_policy_802_1x"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["switch_controller_security_policy_802_1x"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "switch_controller_security_policy_802_1x")
