...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_wireless_controller_mpsk_profile
...
def wireless_controller_mpsk_profile(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_wireless_controller_mpsk_profile_data(wireless_controller_mpsk_profile_data))

    # check_mode starts from here
    if check_mode:
        mkey = fos.get_mkey('wireless_controller', 'mpsk_profile', filtered_data, vdom=vdom)
        current_data = fos.get('wireless_controller', 'mpsk_profile', vdom=vdom, mkey=mkey)
...
def fortios_wireless_controller(data, fos, check_mode):
    ...
    fos.do_member_operation('wireless_controller_mpsk_profile')
    if data['wireless_controller_mpsk_profile']:
        resp = wireless_controller_mpsk_profile(data, fos, check_mode)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('wireless_controller_mpsk_profile'))
    if check_mode:
        return resp
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["wireless_controller_mpsk_profile"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["wireless_controller_mpsk_profile"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "wireless_controller_mpsk_profile")
