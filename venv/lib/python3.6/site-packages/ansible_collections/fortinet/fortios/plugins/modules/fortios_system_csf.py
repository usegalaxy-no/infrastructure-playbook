...
def filter_system_csf_data(json):
    option_list = ['accept_auth_by_cert', 'authorization_request_type', 'certificate',
                   'configuration_sync', 'downstream_access', 'downstream_accprofile',
                   'fabric_connector', 'fabric_device', 'fabric_object_unification',
                   'fabric_workers', 'fixed_key', 'group_name',
                   'group_password', 'log_unification', 'management_ip',
                   'management_port', 'saml_configuration_sync', 'status',
                   'trusted_list', 'upstream_ip', 'upstream_port']
    dictionary = {}
...
def fortios_system(data, fos):
    ...
    fos.do_member_operation('system_csf')
    if data['system_csf']:
