...
def filter_system_dhcp_server_data(json):
    option_list = ['auto_configuration', 'auto_managed_status', 'conflicted_ip_timeout',
                   'ddns_auth', 'ddns_key', 'ddns_keyname',
                   'ddns_server_ip', 'ddns_ttl', 'ddns_update',
                   'ddns_update_override', 'ddns_zone', 'default_gateway',
                   'dhcp_settings_from_fortiipam', 'dns_server1', 'dns_server2',
                   ...
                   'vci_match', 'vci_string', 'wifi_ac_service',
...
def system_dhcp_server(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system_dhcp', 'server', filtered_data, vdom=vdom)
        current_data = fos.get('system_dhcp', 'server', vdom=vdom, mkey=mkey)
...
def main():
    ...
    if module._socket_path:
        ...
        is_error, has_changed, result = fortios_system_dhcp(module.params, fos, module.check_mode)
