...
def filter_system_replacemsg_group_data(json):
    option_list = ['admin', 'alertmail', 'auth',
                   'automation', 'comment', 'custom_message',
                   'device_detection_portal', 'ec', 'fortiguard_wf',
                   'ftp', 'group_type', 'http',
                   'icap', 'mail', 'mm1',
                   'mm3', 'mm4', 'mm7',
                   'mms', 'nac_quar', 'name',
...
def system_replacemsg_group(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'replacemsg_group', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'replacemsg_group', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_replacemsg_group')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_replacemsg_group"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_replacemsg_group"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_replacemsg_group")
