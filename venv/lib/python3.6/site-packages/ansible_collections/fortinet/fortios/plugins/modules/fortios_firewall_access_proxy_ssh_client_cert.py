...
def filter_firewall_access_proxy_ssh_client_cert_data(json):
    option_list = ['auth_ca', 'cert_extension', 'name',
                   'permit_agent_forwarding', 'permit_port_forwarding', 'permit_pty',
                   'permit_user_rc', 'permit_x11_forwarding', 'source_address']
    dictionary = {}
...
def fortios_firewall(data, fos):
    ...
    fos.do_member_operation('firewall_access_proxy_ssh_client_cert')
    if data['firewall_access_proxy_ssh_client_cert']:
        resp = firewall_access_proxy_ssh_client_cert(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('firewall_access_proxy_ssh_client_cert'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_access_proxy_ssh_client_cert"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_access_proxy_ssh_client_cert"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_access_proxy_ssh_client_cert")
