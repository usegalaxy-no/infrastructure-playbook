...
def filter_system_virtual_switch_data(json):
    option_list = ['name', 'physical_switch', 'port',
...
def system_virtual_switch(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_system_virtual_switch_data(system_virtual_switch_data))

    # check_mode starts from here
    if check_mode:
        mkey = fos.get_mkey('system', 'virtual_switch', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'virtual_switch', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_virtual_switch')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_virtual_switch"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_virtual_switch"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_virtual_switch")
