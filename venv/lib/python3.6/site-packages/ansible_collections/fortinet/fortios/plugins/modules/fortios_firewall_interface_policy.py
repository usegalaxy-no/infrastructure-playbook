...
def filter_firewall_interface_policy_data(json):
    option_list = ['address_type', 'application_list', 'application_list_status',
                   ...
                   'scan_botnet_connections', 'service', 'spamfilter_profile',
                   'spamfilter_profile_status', 'srcaddr', 'status',
...
def firewall_interface_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'interface_policy', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'interface_policy', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_interface_policy')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_interface_policy"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_interface_policy"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_interface_policy")
