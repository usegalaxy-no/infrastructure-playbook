...
def filter_user_nac_policy_data(json):
    option_list = ['category', 'description', 'ems_tag',
                   'family', 'firewall_address', 'host',
                   'hw_vendor', 'hw_version', 'mac',
                   'name', 'os', 'src',
                   'ssid_policy', 'status', 'sw_version',
                   'switch_auto_auth', 'switch_fortilink', 'switch_mac_policy',
                   'switch_port_policy', 'switch_scope', 'type',
                   'user', 'user_group']
    dictionary = {}
...
def user_nac_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('user', 'nac_policy', filtered_data, vdom=vdom)
        current_data = fos.get('user', 'nac_policy', vdom=vdom, mkey=mkey)
...
def fortios_user(data, fos, check_mode):
    ...
    fos.do_member_operation('user_nac_policy')
