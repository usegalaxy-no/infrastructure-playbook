...
def filter_vpn_l2tp_data(json):
    option_list = ['compress', 'eip', 'enforce_ipsec',
                   'hello_interval', 'lcp_echo_interval', 'lcp_max_echo_fails',
                   'sip', 'status', 'usrgrp']
    dictionary = {}
...
def fortios_vpn(data, fos):
    ...
    fos.do_member_operation('vpn_l2tp')
    if data['vpn_l2tp']:
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["vpn_l2tp"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "vpn_l2tp")
