...
def filter_system_ipv6_tunnel_data(json):
    option_list = ['auto_asic_offload', 'destination', 'interface',
                   'name', 'source', 'use_sdwan']
    dictionary = {}
...
def system_ipv6_tunnel(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'ipv6_tunnel', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'ipv6_tunnel', vdom=vdom, mkey=mkey)
...
def fortios_system(data, fos, check_mode):
    ...
    fos.do_member_operation('system_ipv6_tunnel')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_ipv6_tunnel"]['options'][attribute_name] = module_spec['options'][attribute_name]
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_ipv6_tunnel")
