...
- collections:
  ...
  vars:
    ...
    vdom: root
  tasks:
  - name: fortios_system_dns_database
...
def filter_system_dns_database_data(json):
    option_list = ['allow_transfer', 'authoritative', 'contact',
                   ...
                   'ip_master', 'ip_primary', 'name',
                   'primary_name', 'rr_max', 'source_ip',
                   'status', 'ttl', 'type',
                   'view']
    dictionary = {}
...
def system_dns_database(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('system', 'dns_database', filtered_data, vdom=vdom)
        current_data = fos.get('system', 'dns_database', vdom=vdom, mkey=mkey)
