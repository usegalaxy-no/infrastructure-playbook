...
def firewall_ip_translation(data, fos, check_mode=False):
    ...
    if check_mode:
        mkey = fos.get_mkey('firewall', 'ip_translation', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'ip_translation', vdom=vdom, mkey=mkey)
...
def fortios_firewall(data, fos, check_mode):
    ...
    fos.do_member_operation('firewall_ip_translation')
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["firewall_ip_translation"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["firewall_ip_translation"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "firewall_ip_translation")
