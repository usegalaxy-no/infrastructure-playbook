...
def filter_system_speed_test_schedule_data(json):
    option_list = ['diffserv', 'dynamic_server', 'interface',
                   'schedules', 'server_name', 'status',
                   'update_inbandwidth', 'update_inbandwidth_maximum', 'update_inbandwidth_minimum',
                   'update_outbandwidth', 'update_outbandwidth_maximum', 'update_outbandwidth_minimum']
    dictionary = {}
...
def fortios_system(data, fos):
    ...
    fos.do_member_operation('system_speed_test_schedule')
    if data['system_speed_test_schedule']:
        resp = system_speed_test_schedule(data, fos)
    else:
        fos._module.fail_json(msg='missing task body: %s' % ('system_speed_test_schedule'))
...
def main():
    ...
    for attribute_name in module_spec['options']:
        fields["system_speed_test_schedule"]['options'][attribute_name] = module_spec['options'][attribute_name]
        if mkeyname and mkeyname == attribute_name:
            fields["system_speed_test_schedule"]['options'][attribute_name]['required'] = True
    ...
    if module._socket_path:
        ...
        versions_check_result = check_schema_versioning(fos, versioned_schema, "system_speed_test_schedule")
